...
public class ChinoTestContent {
	...
	public static boolean validLoggedUser(LoggedUser loggedUser) {
		...
		if(loggedUser.getExpiresIn()!=1800)
			return false;
		if(!validUUID(loggedUser.getUserId()))
			return false;
	...
	public static boolean validLogoutResponse(LogoutResponse user) {
		if(user==null)
			return false;
		if(!validUUID(user.getLogout().getUserId()))
			return false;
		if(user.getLogout().getUsername()==null)
			return false;
		return true;
	}
