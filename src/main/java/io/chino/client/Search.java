...
public class Search extends ChinoBaseAPI{
    ...
    public Search(String hostUrl, Client clientInitialized){
     ...
     */
    public GetDocumentsResponse searchDocuments(SearchRequest searchRequest) throws IOException, ChinoApiException {

        String requestString=mapper.writeValueAsString(searchRequest);
        Response response= client.target(host).path("/search").request().post(Entity.json(requestString));
        JsonNode responseData;
        if(response.getStatus()==200){
            responseData= mapper.readTree(response.readEntity(String.class)).get("data");
        }else{
            throw new ChinoApiException(mapper.readValue(response.readEntity(String.class), ErrorResponse.class));
        }
        if(responseData!=null)
            return mapper.readValue(responseData, GetDocumentsResponse.class);
        return null;
    }
     ...
     * @param withoutIndex the boolean withoutIndex
     ...
     * @throws ChinoApiException
     */
    public GetDocumentsResponse searchDocuments(String schemaId, String resultType, Boolean withoutIndex, String filterType, List<SortOption> sort, List<FilterOption> filter) throws IOException, ChinoApiException {
        SearchRequest searchRequest = new SearchRequest();
        searchRequest.setSchemaId(schemaId);
        searchRequest.setResultType(resultType);
        searchRequest.setWithoutIndex(withoutIndex);
    ...
    {
        searchRequest.setWithoutIndex(value);
        return this;
    }
     ...
     */
    public GetDocumentsResponse search(String schemaId) throws IOException, ChinoApiException {
    ...
    {
        filter.get(filter.indexOf(filterOption)).setCaseSensitive(true);
        searchRequest.setFilter(filter);
        return this;
    }
}
